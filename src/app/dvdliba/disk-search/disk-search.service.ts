...
export class DiskSearchService {
    ...
    extractData = (res: Response): Disk[] => {
        ...
        for (i = 0; i < body.objects.length; i++) {
            // Unstable serach is fixed by commenting the else execution and checking the if/else condition a little bit 
            // to avoid incomplete disk data with "null" cover_url
            if (body.objects[i].cover_url !== null) {
